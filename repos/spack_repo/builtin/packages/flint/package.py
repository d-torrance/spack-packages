...
class Flint(AutotoolsPackage):
    ...
    def fix_in_tree_detection(self):
        ...
        # with both sides resolved instead.  The line is identical in every
        # release that has it, so match on it rather than carry a patch per
        # version.
        #
        # After autoreconf rather than in patch(), so that there is a configure
        # to edit either way: a release ships one, and a git checkout has just
        # had one generated.  Editing configure.ac instead would leave it newer
        # than configure, and flint's maintainer mode rule re-runs bootstrap.sh
        # when that happens, part way through the build.
        filter_file(
